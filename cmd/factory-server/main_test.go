...
func TestReportBrowserRuntimeStartupCheckUsesInstalledModule(t *testing.T) {
	...
	if err := os.WriteFile(filepath.Join(module, "index.js"), []byte(`const {execFileSync}=require("node:child_process");exports.chromium={async launch(options){execFileSync(options.executablePath);return {async newPage(){return {async setContent(){},async title(){return "Factory browser probe"},async close(){}}},async close(){}}}}`), 0o600); err != nil {
		t.Fatal(err)
	}
	...
	if err := controlplane.NewReportBrowserRuntime(launcher, filepath.Join(root, "runtime")).Check(context.Background()); err != nil {
		t.Fatal(err)
	}
