...
class PipelineWatchTests(unittest.TestCase):
    def setUp(self):
        ...
        self.patches = [
            ...
            mock.patch.object(pilot, "api", side_effect=self._api),
        ]
        for patcher in self.patches:
            patcher.start()
            self.addCleanup(patcher.stop)
    ...
    def _api(self, path, body=None):
        if path.startswith("/tasks/"):
            ...
            return self.details.get(task_id, {
                "task": {"context": "Pipeline: Встроенный патруль\n"
                                     "Branch: factory/existing-work"},
                "attempts": [{"result": "BRANCH: factory/existing-work\n"
                                         "Результат предыдущей стадии"}],
            })
        raise AssertionError(path)
    ...
    def test_revive_implement_to_review_carries_branch_and_handoff(self):
        implemented = self.task(stage="Implement")
        implemented.update({"id": "implemented", "created_at": "2026-08-10T12:00:00Z"})
        self.details["implemented"] = {
            "task": {"context": "Pipeline: Встроенный патруль\n"
                                 "Branch: factory/context-branch"},
            "attempts": [{"result": "BRANCH: factory/continued-work\n"
                                     "Реализация и целевые тесты готовы"}],
        }
        self.revive = {"Встроенный патруль": True}

        self.watch([implemented])

        self.assertEqual(len(self.created), 1)
        self.assertIn("[3/3 Review]", self.created[0]["title"])
        self.assertIn("Branch: factory/continued-work", self.created[0]["context"])
        self.assertIn("Реализация и целевые тесты готовы", self.created[0]["context"])

    def test_revive_review_to_verify_carries_branch_and_handoff(self):
        self.conf["stages"].append({"workflow": "Verify"})
        self.workflows["Verify"] = {"enabled": True, "revision_id": "rev-verify"}
        reviewed = self.task(stage="Review")
        reviewed.update({"id": "reviewed", "created_at": "2026-08-10T12:00:00Z"})
        self.details["reviewed"] = {
            "task": {"context": "Pipeline: Встроенный патруль\n"
                                 "Branch: factory/verified-work"},
            "attempts": [{"result": "APPROVE\nЗамечаний к поставке нет"}],
        }
        self.revive = {"Встроенный патруль": True}
        ...
        self.assertEqual(len(self.created), 1)
        self.assertIn("[4/4 Verify]", self.created[0]["title"])
        self.assertIn("Branch: factory/verified-work", self.created[0]["context"])
        self.assertIn("Замечаний к поставке нет", self.created[0]["context"])
    ...
    def test_cycle_consumes_revive_and_starts_the_next_stage(self):
        ...
        def fake_api(path, body=None):
            ...
            if path == "/tasks/finished-spec":
                return {
                    "task": {"context": "Pipeline: Встроенный патруль\n"
                                         "Branch: factory/existing-work",
                             "repository_id": "repo-id"},
                    "workflow": {"title": "Specification"},
                    "attempts": [{"result": "BRANCH: factory/existing-work\n"
                                             "Спецификация готова\n"
                                             "ГОТОВО-КОГДА: следующий этап создан"}],
                }
            raise AssertionError(path)
        ...
        with contextlib.ExitStack() as stack:
            ...
            stack.enter_context(mock.patch.object(
                pilot, "decide",
                return_value={"action": "advance", "handoff": "Спецификация принята"},
            ))
            for name in noops:
                stack.enter_context(mock.patch.object(pilot, name))
