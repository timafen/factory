...
"""
    ...
    def test_same_title_work_ids_keep_artifacts_isolated_through_restart_and_merge(self):
        ...
        with contextlib.ExitStack() as stack:
            ...
            stack.enter_context(mock.patch.object(
                ...
                {"back": False,
                 ...
                 else "factory/b-clean",
                 "head": "c" * 40 if branch == "factory/a-implementation"
                 else "d" * 40, "note": "clean"}))
            stack.enter_context(mock.patch.object(
                pilot, "verify_gate", side_effect=lambda _repo, branch: {
                    "snapshot": {"candidate_sha": "c" * 40
                                 if branch == "factory/a-clean" else "d" * 40}}))
            stack.enter_context(mock.patch.object(
                pilot, "_verified_merge_result", return_value=False))
            stack.enter_context(mock.patch.object(
                pilot, "_merged_commit_sha", return_value="merged-sha"))
            merge = stack.enter_context(mock.patch.object(
                pilot, "gh_merge", return_value=(True, "merged")))
            for name in noops:
                stack.enter_context(mock.patch.object(pilot, name))
            stack.enter_context(mock.patch.object(pilot, "deploy_after_merge", return_value=None))
            ...
            pilot.cycle(dict(self.conf, auto_merge=True, max_terminal_tasks_per_cycle=10), state)
            self.assertEqual(
        ...
        self.assertEqual(merge.call_args_list, [
            mock.call("github.com/acme/repo", "factory/a-clean", base, "c" * 40),
            mock.call("github.com/acme/repo", "factory/b-clean", base, "d" * 40),
        ])
