...
PATROL_MODEL_MAX_OBSERVATION_GAP_SECONDS = 60 * 60
...
def add_patrol_model_reference_observation(audit, input_bundle, terra_findings,
    ...
    observation = add_patrol_model_observation(
        audit, input_bundle, labelled_terra, labelled_sol, observed_at)
...
def evaluate_patrol_model_pair_run(audit, now=None):
    ...
    terminal = audit.get("decision") in ("approved", "rejected")
    evaluation_time = audit.get("evaluated_at") if terminal else now
    result = dict(audit)
    observations = audit.get("observations") or []
    reasons = []
    try:
        started_at = float(audit["started_at"])
        ends_at = float(audit["ends_at"])
        evaluated_at = float(evaluation_time)
    except (KeyError, TypeError, ValueError, OverflowError):
        started_at, ends_at, evaluated_at = 0.0, float("inf"), now
        reasons.append("invalid audit window")
    if ends_at - started_at != PATROL_MODEL_EVALUATION_SECONDS:
        reasons.append("audit window is not exactly 24 hours")
    completed = evaluated_at >= ends_at and now >= ends_at
    if not completed:
    ...
    for observation in observations:
        try:
            ...
            if observed_at < started_at or observed_at > ends_at:
                raise ValueError
            observation_times.append(observed_at)
        except (KeyError, TypeError, ValueError, OverflowError):
            reasons.append("invalid observation timestamp")
    ...
    if (not observation_times or observation_times[0] > started_at or
            observation_times[-1] < ends_at or
            any(later - earlier > PATROL_MODEL_MAX_OBSERVATION_GAP_SECONDS
                for earlier, later in zip(observation_times, observation_times[1:]))):
        reasons.append("observations do not continuously cover the 24-hour window")
    ...
    result.update({"metrics": {"useful_retention": retention,
                   ...
                   "reasons": reasons})
    if terminal:
        consistent = all(audit.get(field) == result.get(field) for field in
                         ("decision", "effective_model", "metrics", "reasons"))
        if not consistent:
            result.update({"decision": "rejected", "effective_model": PATROL_MODEL_SOL,
                           "evaluated_at": now,
                           "reasons": list(dict.fromkeys(
                               reasons + ["stored audit decision is inconsistent"]))})
    return result
