...
def orchestrator_answer(conf, stage, base, situation, question, prior_result,
    ...
    prompt = (
        ...
        "health, logs, restart, manage, sandbox, env-names, release-info. "
...
def admin_fx_argv(action):
    ...
    if verb in simple and not args:
        pass
    elif verb == "logs" and len(args) <= 1 and (not args or re.fullmatch(r"[1-9][0-9]{0,2}", args[0])):
        pass
    elif verb == "restart" and args in ([], ["gunicorn"], ["worker"], ["notifications"], ["order-reconcile"], ["all"]):
        pass
    elif verb == "manage" and args in (["check"], ["migrate"], ["showmigrations"], ["collectstatic"]):
        pass
    elif verb == "sandbox" and args and args[0] in ("bootstrap-accounts", "seller-policies", "listings") and all(
            re.fullmatch(r"--(?:tenant-id|account-id|fulfillment-policy-id|payment-policy-id|return-policy-id|listing-count)=[A-Za-z0-9_.-]+|--dry-run|--force|--interactive-bootstrap|--role=seller|--consent-status=[A-Za-z0-9_.-]+", arg)
            for arg in args[1:]):
        pass
    else:
