...
def previous_stage_handoff(tasks, base, src):
    """Load the source task exactly as the regular cycle does before advancing.

    A task-list item has neither the stage result nor its input context.  Both
    are needed on revive: the result is the lossless handoff and either place
    may contain the working branch.
    """
    detail = api(f"/tasks/{src['id']}")
    task = detail.get("task") or {}
    previous_context = detail.get("context") or task.get("context") or ""
    attempts = detail.get("attempts") or []
    result = next(
        (attempt.get("result") for attempt in reversed(attempts)
         if attempt.get("result")),
    ...
    branch = extract_branch(result, previous_context)
    if not branch:
    ...
    for base, lst in groups.items():
        ...
        if retry_stage:
            ...
            recovery_context = ("Конвейер встал: предыдущий этап закончился, "
                                ...
                                "заново.")
        try:
            branch, previous_result = previous_stage_handoff(tasks, base, src)
        except Exception as error:
            # Without the source details revive cannot safely continue existing
            # work. Keep its signal in the spool and retry on the next cycle.
            log("watch_source_detail_error", repr(error))
            continue
        must_keep_branch = (
            nxt in ("Review", "Verify")
            or (retry_stage is not None and stages.index(nxt) > 0)
        )
        if base in revive and must_keep_branch and not branch:
            log(f"WATCH ждёт ветку для продолжения {base!r}: {previous} -> {nxt}")
            continue
        branch_line = f"Branch: {branch}\n" if branch else ""
        # The decision-model handoff used by the regular cycle is not stored.
        # Carrying the full source result is lossless and gives the revived
        # stage the same report and branch that a normal transition receives.
        context = (
            f"Pipeline: {base}\nPrevious stage: {previous}\n{branch_line}"
            f"Orchestrator handoff: {squeeze(previous_result, 4000)}\n\n"
            f"Отчёт предыдущей стадии (сокращён):\n{squeeze(previous_result)}\n\n"
            f"{recovery_context}"
        )[:20000]
        try:
...
def cycle(conf, state):
    ...
    watch_advanced_task_ids = set()
    try:
        watch_advanced_task_ids = pipeline_watch(
            conf, tasks, workflows, workers
        ) or set()
    except Exception as e:
    ...
    for t in tasks:
        ...
        if not title.startswith(PREFIX):
            continue
        # pipeline_watch already created this task's continuation from the same
        # API snapshot. The next cycle will see that continuation through the
        # normal duplicate guard; processing the stale source again now would
        # create the same stage twice.
        if tid in watch_advanced_task_ids:
            continue
