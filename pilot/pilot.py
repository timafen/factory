...
def latest_successful_transition(stage_tasks, stages):
    ...
    if not candidates:
        return -1, None
    dated = [item for item in candidates if item[1].get("created_at")]
    if dated:
        stage, task = max(
            dated,
            key=lambda item: (
        ...
        # Compatibility callers may omit timestamps. Preserve the former
        # furthest-stage behaviour only when chronology is unavailable.
        stage, task = max(candidates, key=lambda item: stages.index(item[0]))
    return stages.index(stage), task
    ...
    for t in tasks:
        ...
        if tstate != "succeeded":
            ...
            if INFRA_SIGNS.search(err or "") and cap_rescues(base, "INFRA") < 3:
                ...
                try:
                    revived_index = revived_stage_index(t)
                    retry_key = str(uuid.uuid4())
                    if revived_index is not None:
                        retry_key = (AUTO_REVIVE_REQUEST_PREFIX
                                     + str(revived_index) + ":" + retry_key)
                    create_task({"request_key": retry_key,
                                 "title": title,
        ...
        # Keep the stage marker in the display title when it fits. A revived
        # work may already use the full title allowance; in that case its exact
        # owner-visible name is the identity, and the stage belongs in metadata.
        prefixed_next_title = f"[auto] [{idx + 2}/{len(stages)} {next_stage}] {base}"
        next_title = prefixed_next_title
        next_request_key = str(uuid.uuid4())
        if len(prefixed_next_title) > 200:
            next_title = base
            next_request_key = (AUTO_REVIVE_REQUEST_PREFIX + str(idx + 1)
                                + ":" + next_request_key)
