...
func VerifyProject(ctx context.Context, config Config, options ProjectVerificationOptions) error {
	...
	credential, err := loadWorkerCredential(config.DataDirectory)
	if err != nil {
		return err
	}
	if credential == "" {
		return errors.New("worker has no server-issued credential; start it and complete registration first")
	}
	client := newClient(config.Server, nil)
	client.setWorkerCredential(credential)
