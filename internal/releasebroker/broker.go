...
func NewAt(stateDir string, executor Executor) (*Broker, error) {
	...
	for _, entry := range entries {
		...
		// Executors with a durable delivery status can be observed by GET below;
		// legacy executors still fail closed because their child cannot be proven.
		if item.Status == "launching" || item.Status == "running" {
			if _, ok := executor.(DeliveryStatusExecutor); ok {
				item.Status = "running"
				b.refresh(&item)
			} else {
				updated := item
