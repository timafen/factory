...
func TestMissingBeforeCaptureContinuesToBlockTaskClaim(t *testing.T) {
	store := newTestStore(t)
	worker := registerTestWorker(t, store, workerA, 1, protocol.RepositoryRegistration{Key: "factory", RemoteIdentity: "github.com/example/before-capture"})
	task := createTestTask(t, store, "before-capture", workerA, worker.Repositories[0].ID)
	if _, err := store.db.Exec(`INSERT INTO task_visual_targets(work_id,url,state_text,viewport_width,viewport_height,created_at) VALUES(?,?,?,?,?,?)`, task.Task.WorkID, "https://factory.timafen.com/listings", "Готово", 800, 600, store.now().UnixMilli()); err != nil {
		t.Fatal(err)
	}
	if _, err := store.db.Exec(`INSERT INTO visual_captures(work_id,phase,status,updated_at) VALUES(?,'before','missing',?)`, task.Task.WorkID, store.now().UTC().Format(time.RFC3339Nano)); err != nil {
		t.Fatal(err)
	}
	claim, err := store.Claim(context.Background(), workerA, protocol.ClaimRequest{RequestID: "before-capture-claim", LeaseToken: tokenA})
	if err != nil {
		t.Fatal(err)
	}
	if claim != nil {
		t.Fatalf("task started after missing before capture: %#v", claim)
	}
}
