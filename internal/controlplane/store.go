...
// or rotates it when a direct registration cannot confirm one. Rotation makes
// an initial lost response or local persistence failure recoverable on retry.
func (s *Store) RefreshWorkerCredential(ctx context.Context, workerID, presented string) (string, error) {
	if presented != "" {
		if err := s.AuthenticateWorkerCredential(ctx, workerID, presented); err == nil {
			...
			if !errors.As(err, &service) || service.Status != 401 {
				return "", err
			}
		}
	}
	return s.IssueWorkerCredential(ctx, workerID)
}
