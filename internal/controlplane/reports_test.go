...
func TestDailyReportRebuildsWhenLateAfterCaptureArrives(t *testing.T) {
	...
	if _, err := store.db.Exec(`UPDATE visual_captures SET status='ready',path=?,sha256=?,captured_at=?,updated_at=? WHERE work_id=? AND phase='after'`, capturePath, hash, stamp, stamp, task.Task.WorkID); err != nil {
		t.Fatal(err)
	}
	if err := service.createPreviousDay(context.Background()); err != nil {
		t.Fatal(err)
	...
	if len(renderer.documents) != 2 || !strings.Contains(renderer.documents[1], "Снимок после</figcaption><img") {
