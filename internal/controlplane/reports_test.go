...
func (renderer *recordingReportRenderer) Render(_ context.Context, document, output string) error {
	renderer.documents = append(renderer.documents, document)
	return os.WriteFile(output, []byte(fmt.Sprintf("%%PDF-render-%d", len(renderer.documents))), 0o600)
}

func TestDailyReportRebuildsWhenLateAfterCaptureArrives(t *testing.T) {
	store := newTestStore(t)
	now := time.Date(2026, 8, 13, 9, 0, 0, 0, time.UTC)
	store.now = func() time.Time { return now }
	worker := registerTestWorker(t, store, workerA, 1, protocol.RepositoryRegistration{Key: "factory", RemoteIdentity: "github.com/example/late-after"})
	task := createTestTask(t, store, "late-after", workerA, worker.Repositories[0].ID)
	created := now.AddDate(0, 0, -1).UnixMilli()
	if _, err := store.db.Exec(`INSERT INTO task_visual_targets(work_id,url,state_text,viewport_width,viewport_height,created_at) VALUES(?,?,?,?,?,?)`, task.Task.WorkID, "https://factory.timafen.com/listings", "Готово", 800, 600, created); err != nil {
		t.Fatal(err)
	}
	stamp := now.UTC().Format(time.RFC3339Nano)
	if _, err := store.db.Exec(`INSERT INTO visual_captures(work_id,phase,status,updated_at) VALUES(?,'before','missing',?)`, task.Task.WorkID, stamp); err != nil {
		t.Fatal(err)
	}
	if _, err := store.db.Exec(`INSERT INTO visual_captures(work_id,phase,status,updated_at) VALUES(?,'after','pending',?)`, task.Task.WorkID, stamp); err != nil {
		t.Fatal(err)
	}
	root := t.TempDir()
	renderer := &recordingReportRenderer{}
	service := &DailyReportService{store: store, logger: slog.Default(), renderer: renderer, root: root, location: time.UTC, renderTimeout: time.Second}
	if err := service.createPreviousDay(context.Background()); err != nil {
		t.Fatal(err)
	}
	var firstPath, firstSource string
	if err := store.db.QueryRow(`SELECT pdf_path,source_sha256 FROM daily_reports WHERE report_date='2026-08-12' AND timezone='UTC'`).Scan(&firstPath, &firstSource); err != nil {
		t.Fatal(err)
	}
	capturePath := filepath.Join("captures", "late-after.png")
	if err := os.MkdirAll(filepath.Join(root, "captures"), 0o700); err != nil {
		t.Fatal(err)
	}
	image := []byte("\x89PNG\r\n\x1a\nlate-after")
	if err := os.WriteFile(filepath.Join(root, capturePath), image, 0o600); err != nil {
		t.Fatal(err)
	}
	hash := fmt.Sprintf("%x", sha256.Sum256(image))
	now = now.AddDate(0, 0, 2) // The late after capture arrives after its report day is no longer yesterday.
	lateStamp := now.UTC().Format(time.RFC3339Nano)
	if _, err := store.db.Exec(`UPDATE visual_captures SET status='ready',path=?,sha256=?,captured_at=?,updated_at=? WHERE work_id=? AND phase='after'`, capturePath, hash, lateStamp, lateStamp, task.Task.WorkID); err != nil {
		t.Fatal(err)
	}
	service.runOnce(context.Background())
	var secondPath, secondSource string
	if err := store.db.QueryRow(`SELECT pdf_path,source_sha256 FROM daily_reports WHERE report_date='2026-08-12' AND timezone='UTC'`).Scan(&secondPath, &secondSource); err != nil {
		t.Fatal(err)
	}
	if len(renderer.documents) != 3 || !strings.Contains(renderer.documents[2], "Снимок после</figcaption><img") {
		t.Fatalf("late capture did not rebuild report: renders=%d", len(renderer.documents))
	}
	if firstSource == secondSource || firstPath == secondPath {
		t.Fatalf("report inputs were not versioned: source %q -> %q, path %q -> %q", firstSource, secondSource, firstPath, secondPath)
	}
	if _, err := os.Stat(filepath.Join(root, firstPath)); !os.IsNotExist(err) {
		t.Fatalf("superseded PDF was not removed: %v", err)
	}
	service.runOnce(context.Background())
	if len(renderer.documents) != 3 {
		t.Fatalf("unchanged late capture rendered again: %d", len(renderer.documents))
	}
}
