...
func registerReadyProjectWorker(t *testing.T, store *Store, project protocol.Project) {
	t.Helper()
	_, err := store.RegisterWorker(context.Background(), "project-ready-worker", projectWorkerRegistration("project-ready-worker", project))
	if err != nil {
		t.Fatal(err)
	}
}

func projectWorkerRegistration(name string, project protocol.Project) protocol.WorkerRegistration {
...
func TestProductionStaysBlockedAndRejectsClientOwnerConfirmation(t *testing.T) {
	store := newTestStore(t)
	...
	_, err = store.RunProjectOperation(context.Background(), &fakeProjectRunner{}, fakeHealth{}, project.ID, "production", "release", protocol.ProjectOperationRequest{CommitSHA: projectSHA})
	if errorCode(err) != "production_confirmation_required" {
		t.Fatalf("production release was not blocked: %v", err)
	}

	request := httptest.NewRequest(http.MethodPost, "/api/v1/projects/ignored/environments/production/release", bytes.NewBufferString(`{"commit_sha":"`+projectSHA+`","owner_confirmed":true}`))
	request.Header.Set("Content-Type", "application/json")
	request.SetPathValue("project_id", project.ID)
	request.SetPathValue("environment", "production")
	request.SetPathValue("operation", "release")
	response := httptest.NewRecorder()
	api := &API{store: store, projectRunner: &fakeProjectRunner{}, projectHealth: fakeHealth{}}
	api.runProjectOperation(response, request)
	if response.Code != http.StatusBadRequest || !strings.Contains(response.Body.String(), "malformed_json") {
		t.Fatalf("client owner_confirmed status=%d body=%s", response.Code, response.Body.String())
	}
}
