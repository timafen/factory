...
func TestDialogDeliversScreenshotAndRemovesTemporaryFile(t *testing.T) {
	...
	if _, err := os.Stat(filepath.Dir(runner.screenshotPath)); !errors.Is(err, os.ErrNotExist) {
		t.Fatalf("isolated screenshot directory remains: %v", err)
	}
}

func TestClaudeDialogAllowsOnlyIsolatedScreenshotDirectory(t *testing.T) {
	foreignPath := filepath.Join(os.TempDir(), "factory-dialog-foreign-file")
	if err := os.WriteFile(foreignPath, []byte("must stay inaccessible"), 0o600); err != nil {
		t.Fatal(err)
	}
	t.Cleanup(func() { _ = os.Remove(foreignPath) })

	png := []byte{0x89, 'P', 'N', 'G', '\r', '\n', 0x1a, '\n', 0, 0, 0, 0}
	screenshotPath, err := materializeDialogScreenshot(&protocol.DialogScreenshot{
		Name: "screen.png", ContentType: "image/png", Data: base64.StdEncoding.EncodeToString(png),
	})
	if err != nil {
		t.Fatal(err)
	}
	t.Cleanup(func() { _ = os.RemoveAll(filepath.Dir(screenshotPath)) })
	if filepath.Dir(screenshotPath) == os.TempDir() {
		t.Fatalf("screenshot was placed directly in system temp directory: %q", screenshotPath)
	}
	...
	if _, err := os.Stat(filepath.Join(filepath.Dir(screenshotPath), filepath.Base(foreignPath))); !errors.Is(err, os.ErrNotExist) {
		t.Fatalf("foreign system-temp file is visible through allowed directory: %v", err)
	}
}
