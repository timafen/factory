...
func TestDialogRejectsInvalidScreenshotsBeforeRunner(t *testing.T) {
	oversizedPNG := append([]byte("\x89PNG\r\n\x1a\n"), make([]byte, dialogMaxScreenshotBytes-7)...)
	for name, screenshot := range map[string]string{
		...
		"too large":        `{"name":"x.png","content_type":"image/png","data":"` + base64.StdEncoding.EncodeToString(oversizedPNG) + `"}`,
	} {
