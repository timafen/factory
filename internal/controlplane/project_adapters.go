...
func (s *Store) ProjectOperation(ctx context.Context, runner projectCommandRunner, health projectHealthChecker, projectID, operationID string) (protocol.ProjectOperation, error) {
	...
	case "succeeded":
		if err := health.Check(ctx, environment.HealthURL, environment.WebHosts); err == nil {
			return s.finishProjectOperation(ctx, operation, "succeeded", "external self-release completed and health was verified after the server restart")
		}
		_, processEnvironment, resolveErr := s.resolveProjectSecretEnvironment(project, operation.Environment)
		if resolveErr != nil {
			return s.finishProjectOperation(ctx, operation, "rollback_failed", "self-release health check failed and rollback secrets could not be resolved")
		}
		if _, rollbackErr := runProjectAdapter(ctx, runner, environment.RollbackAdapter, operation.CommitSHA, processEnvironment); rollbackErr != nil {
			return s.finishProjectOperation(ctx, operation, "rollback_failed", "self-release health check and named rollback failed")
		}
		if rollbackHealthErr := health.Check(ctx, environment.HealthURL, environment.WebHosts); rollbackHealthErr != nil {
			return s.finishProjectOperation(ctx, operation, "rollback_failed", "self-release health check failed; named rollback completed but restored health could not be verified")
		}
		return s.finishProjectOperation(ctx, operation, "health_failed_rolled_back", "self-release health check failed; named rollback and restored health were verified")
	case "release_failed_rolled_back":
		if err := health.Check(ctx, environment.HealthURL, environment.WebHosts); err != nil {
			return s.finishProjectOperation(ctx, operation, "rollback_failed", "external self-release failed; broker reported automatic rollback but restored health could not be verified")
		}
		return s.finishProjectOperation(ctx, operation, "release_failed_rolled_back", "external self-release failed; automatic rollback and restored health were verified")
	case "rollback_failed":
...
func (s *Store) RunProjectOperation(ctx context.Context, runner projectCommandRunner, health projectHealthChecker, projectID, environmentName, kind string, request protocol.ProjectOperationRequest) (protocol.ProjectOperation, error) {
	...
	if kind == "release" && adapter == "fx-factory-release" {
		if err := runner.StartDurable(ctx, factoryProjectOperationUnit(operation.ID), "/usr/local/bin/fx", []string{"factory", "release", request.CommitSHA}); err != nil {
			return s.updateRunningProjectOperation(ctx, operation, "external release broker POST result is unknown; poll the broker before starting another release or rollback")
		}
		return s.updateRunningProjectOperation(ctx, operation, "external self-release started; poll this operation after the server restart")
	}
		...
		if kind == "release" {
			if automatic {
				...
				if stateErr == nil && currentState == rollbackState {
					if healthErr := health.Check(ctx, environment.HealthURL, environment.WebHosts); healthErr == nil {
						return s.finishProjectOperation(ctx, operation, "release_failed_rolled_back", "release failed; automatic rollback and restored health were verified")
					}
					return s.finishProjectOperation(ctx, operation, "rollback_failed", "release failed; automatic rollback target was restored but health could not be verified")
				}
			...
			if healthErr := health.Check(ctx, environment.HealthURL, environment.WebHosts); healthErr != nil {
				return s.finishProjectOperation(ctx, operation, "rollback_failed", "release failed; named rollback completed but restored health could not be verified")
			}
			return s.finishProjectOperation(ctx, operation, "release_failed_rolled_back", "release failed; named rollback and restored health were verified")
		}
	...
	if kind == "release" {
		if err := health.Check(ctx, environment.HealthURL, environment.WebHosts); err != nil {
			...
			if rollbackHealthErr := health.Check(ctx, environment.HealthURL, environment.WebHosts); rollbackHealthErr != nil {
				return s.finishProjectOperation(ctx, operation, "rollback_failed", "health check failed; named rollback completed but restored health could not be verified")
			}
			return s.finishProjectOperation(ctx, operation, "health_failed_rolled_back", "health check failed; named rollback and restored health were verified")
		}
	} else if err := health.Check(ctx, environment.HealthURL, environment.WebHosts); err != nil {
		return s.finishProjectOperation(ctx, operation, "rollback_failed", "named rollback completed but health could not be verified")
	}
	label := "Release"
	if kind == "rollback" {
		label = "Rollback"
