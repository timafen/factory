...
func TestFactoryBrokerLostPOSTResponseStaysRunningUntilGETConfirmsOutcome(t *testing.T) {
	...
	polled, err := store.ProjectOperation(context.Background(), runner, project.ID, operation.ID)
	...
	completed, err := store.ProjectOperation(context.Background(), runner, project.ID, operation.ID)
