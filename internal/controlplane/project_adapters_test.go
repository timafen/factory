...
func TestTarserHealthFailureRunsAndVerifiesNamedRollback(t *testing.T) {
	...
	if operation.Status != "health_failed_rolled_back" || len(runner.calls) != 4 || health.calls != 2 {
...
func TestFactorySelfReleaseHealthFailureRollsBackAndVerifiesRestoredHealth(t *testing.T) {
	...
	if completed.Status != "health_failed_rolled_back" || health.calls != 2 {
		t.Fatalf("operation=%+v health calls=%d", completed, health.calls)
	}
	wantRollback := recordedProjectCommand{executable: "/usr/local/bin/fx", args: []string{"factory", "rollback"}, environment: []string{"GITHUB_TOKEN=value"}}
...
func TestManualRollbackRequiresHealthyResultBeforeSuccess(t *testing.T) {
	for _, test := range []struct {
		...
		t.Run(test.name, func(t *testing.T) {
			...
			operation, err := store.RunProjectOperation(context.Background(), &fakeProjectRunner{}, health, project.ID, "staging", "rollback", structProjectOperationRequest())
