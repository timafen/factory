...
func TestPilotConfigStoreAtomicallyReplacesFileAndKeepsOriginalOnWriteFailure(t *testing.T) {
	store, path := writePilotFixture(t, validPilotSettings())
	...
	store.writeFile = func([]byte) error { return errors.New("injected write failure") }
	saved.Settings.PollSeconds = 20
	if _, err := store.Write(saved.Version, saved.Settings); err == nil {
	...
	if !bytes.Equal(afterFailure, savedBody) {
		t.Fatal("write failure changed the previous config")
	}
}
