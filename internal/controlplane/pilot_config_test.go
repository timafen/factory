...
func TestReviveStuckDoesNotRewriteConfigWhenPilotReadsSignalImmediately(t *testing.T) {
	store, path := writePilotFixture(t, validPilotSettings())
	if err := os.WriteFile(store.stallPath(), []byte(`{"Застрявшая":{"why":"give_up"}}`), 0o600); err != nil {
		t.Fatal(err)
	}
	published := make(chan string)
	consumed := make(chan struct{})
	store.reviveSignalPublished = func(work string) {
		published <- work
		<-consumed
	}
	result := make(chan error, 1)
	go func() {
		_, err := store.Revive("Застрявшая")
		result <- err
	}()

	work := <-published
	body, err := os.ReadFile(store.reviveSignalPath(work))
	if err != nil || string(body) != work {
		t.Fatalf("pilot read signal = %q, %v", body, err)
	}
	if err := os.Rename(store.reviveSignalPath(work), store.reviveReceiptPath(work)); err != nil {
		t.Fatal(err)
	}
	// Make any attempted config rewrite fail after the pilot has consumed the
	// command. Reviving a stuck work must still report success.
	...
	if err := <-result; err != nil {
		t.Fatalf("revive after immediate pilot read: %v", err)
	}
}
