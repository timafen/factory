...
type commandDailyReportRenderer struct {
	script  string
	runtime ReportBrowserRuntime
}
...
func (r commandDailyReportRenderer) Render(ctx context.Context, document, output string) error {
	...
	command.Env = r.runtime.commandEnvironment()
...
func NewDailyReportService(store *Store, logger *slog.Logger, root, rendererScript, timezone string, runtime ReportBrowserRuntime) (*DailyReportService, error) {
	location, err := time.LoadLocation(timezone)
	if err != nil {
		return nil, fmt.Errorf("load report timezone: %w", err)
	}
	store.reportRoot = root
	return &DailyReportService{store: store, logger: logger, renderer: commandDailyReportRenderer{script: rendererScript, runtime: runtime}, root: root, location: location, checkEvery: time.Minute, renderTimeout: 10 * time.Minute}, nil
}
...
func (service *DailyReportService) runOnce(ctx context.Context) {
	...
	for _, date := range service.store.reportDatesWithLateCaptures(ctx, service.location) {
		if err := service.createReportDate(ctx, date); err != nil && ctx.Err() == nil {
			service.logger.Error("daily_report_rebuild_failed", "date", date, "error", err)
		}
	}
}
...
func (service *DailyReportService) createPreviousDay(ctx context.Context) error {
	...
	return service.createReportDate(ctx, date)
}

func (service *DailyReportService) createReportDate(ctx context.Context, date string) error {
	...
	sourceJSON, err := json.Marshal(struct {
		...
		Works   []reportVisualWork `json:"works"`
	}{Metrics: metricsJSON, Works: works})
	if err != nil {
		return err
	}
	sourceHash := fmt.Sprintf("%x", sha256.Sum256(sourceJSON))
	token, previousPath, err := service.store.claimDailyReport(ctx, date, service.location.String(), sourceHash)
	if err != nil || token == "" {
		return err
	}
	...
	result, err := service.store.db.ExecContext(ctx, `UPDATE daily_reports SET status='ready',metrics_json=?,pdf_path=?,pdf_sha256=?,pdf_size=?,source_sha256=?,error='',updated_at=? WHERE report_date=? AND timezone=? AND status='running' AND claim_token=?`, string(metricsJSON), filename, hash, len(content), sourceHash, service.store.now().UTC().Format(time.RFC3339Nano), date, service.location.String(), token)
	if err != nil {
		_ = os.Remove(final)
		return err
	}
	...
	if previousPath != "" && previousPath != filename {
		_ = os.Remove(filepath.Join(service.root, filepath.Base(previousPath)))
	}
	return nil
}

// reportDatesWithLateCaptures returns existing report days whose ready visual
// input was published after the last PDF. This lets an after screenshot that
// arrives days late rebuild its own report instead of waiting for a new day.
func (s *Store) reportDatesWithLateCaptures(ctx context.Context, location *time.Location) []string {
	rows, err := s.db.QueryContext(ctx, `SELECT target.created_at,capture.updated_at FROM task_visual_targets target JOIN visual_captures capture ON capture.work_id=target.work_id WHERE capture.status='ready'`)
	if err != nil {
		return nil
	}
	defer rows.Close()
	...
	for rows.Next() {
		var createdAt int64
		var captureUpdated string
		if rows.Scan(&createdAt, &captureUpdated) != nil {
			continue
		}
		date := time.UnixMilli(createdAt).In(location).Format(time.DateOnly)
		var reportUpdated string
		if err := s.db.QueryRowContext(ctx, `SELECT updated_at FROM daily_reports WHERE report_date=? AND timezone=? AND status='ready'`, date, location.String()).Scan(&reportUpdated); err != nil {
			continue
		}
		captureTime, captureErr := time.Parse(time.RFC3339Nano, captureUpdated)
		reportTime, reportErr := time.Parse(time.RFC3339Nano, reportUpdated)
		if captureErr == nil && reportErr == nil && captureTime.After(reportTime) {
			dates[date] = true
		}
	}
...
func (s *Store) claimDailyReport(ctx context.Context, date, timezone, sourceHash string) (string, string, error) {
	instant := s.now().UTC()
	...
	result, err := s.db.ExecContext(ctx, `UPDATE daily_reports SET status='running',claim_token=?,error='',updated_at=? WHERE report_date=? AND timezone=? AND (status IN ('pending','error') OR (status='ready' AND source_sha256<>?) OR (status='running' AND updated_at<?))`, token, now, date, timezone, sourceHash, instant.Add(-time.Hour).Format(time.RFC3339Nano))
	if err != nil {
		return "", "", unavailable(err)
	}
	changed, err := result.RowsAffected()
	...
	if err := s.db.QueryRowContext(ctx, `SELECT pdf_path FROM daily_reports WHERE report_date=? AND timezone=? AND status='running' AND claim_token=?`, date, timezone, token).Scan(&previousPath); err != nil {
		return "", "", unavailable(err)
	}
	return token, previousPath, nil
}
