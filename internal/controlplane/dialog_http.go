...
func serializeDialog(messages []protocol.DialogMessage) string {
	...
	b.WriteString("Продолжи разговор. Ответь только на последний вопрос. Если нужно увидеть тестовый стенд глазами, используй /opt/factory-data/bin/factory-worker browser; он разрешает только https://staging-automation.tarser.net и его пути.\n\n")
