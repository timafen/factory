...
type commandVisualCapturer struct {
	script  string
...
func (capturer commandVisualCapturer) Capture(ctx context.Context, target protocol.VisualTarget, output string) error {
	...
	command.Env = capturer.runtime.commandEnvironment()
...
func NewVisualCaptureService(store *Store, logger *slog.Logger, root, script string, runtime ReportBrowserRuntime) *VisualCaptureService {
	store.reportRoot = root
	return &VisualCaptureService{store: store, logger: logger, capturer: commandVisualCapturer{script: script, runtime: runtime}, root: root, checkEvery: time.Minute, captureTimeout: 2 * time.Minute}
}
