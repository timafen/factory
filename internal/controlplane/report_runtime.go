...
func (runtime ReportBrowserRuntime) Check(ctx context.Context) error {
	...
	probe := `const {createRequire}=require("node:module");const p=require("node:path");const r=createRequire(p.join(process.argv[1],"package.json"));const {chromium}=r("playwright");if(!chromium||typeof chromium.launch!=="function")process.exit(2)`
	command := exec.CommandContext(ctx, "node", "-e", probe, runtime.Root)
