...
const (
	launcherShutdownTimeout = 3 * time.Second
...
// runProcessGroup puts the sudo/fx/launcher wrapper in its own process group.
// Chromium deliberately starts a separate session inside the launcher. On
// cancellation Go only asks the launcher to stop; the root-owned launcher is
// the single owner of Chromium termination and namespace cleanup.
func runProcessGroup(ctx context.Context, command *exec.Cmd, afterStart func() error) ([]byte, error) {
	...
	if afterStart != nil {
		if err := afterStart(); err != nil {
			_ = stopLauncher(command.Process.Pid, nil)
			_ = command.Wait()
	...
	if err := stopLauncher(command.Process.Pid, done); err != nil {
		return nil, errors.Join(ctx.Err(), err)
	}
	return []byte(output.String()), ctx.Err()
}

func stopLauncher(pid int, done <-chan error) error {
	// The negative PID reaches sudo, fx and the launcher, but not Chromium:
	// setsid in factory-browser-sandbox gives Chromium its own process group.
	if err := syscall.Kill(-pid, syscall.SIGTERM); err != nil && !errors.Is(err, syscall.ESRCH) {
		return fmt.Errorf("request browser launcher shutdown: %w", err)
	}
	if done == nil {
		return nil
	}
	timer := time.NewTimer(launcherShutdownTimeout)
	defer timer.Stop()
	select {
	case <-done:
		return nil
	case <-timer.C:
		return errors.New("browser launcher did not finish its cleanup")
	}
}
