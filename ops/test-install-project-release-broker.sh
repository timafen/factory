...
grep -qx 'StateDirectory=factory/release-broker factory/release-deliveries' "$temporary/systemd/factory-release-broker.service"
grep -q 'delivery status directory is a symlink' "$root/ops/fx-factory-release"
grep -q 'delivery status directory is not root-owned' "$root/ops/fx-factory-release"
grep -q 'delivery status directory is not mode 0700' "$root/ops/fx-factory-release"
