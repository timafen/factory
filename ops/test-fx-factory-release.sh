...
make_fixture() {
  ...
  chmod +x "$case_dir/system/fx" "$case_dir/system/fx-factory-release" "$case_dir/system/libexec/"*
...
case "$*" in
  *'clone --quiet'*)
    ...
    for file in fx-factory-release install-server-browser.sh factory-browser-sandbox test-browser-sandbox.sh; do
...
for path in fx fx-factory-release browser-sandbox libexec/factory-browser-sandbox libexec/factory-browser-sandbox-check; do
