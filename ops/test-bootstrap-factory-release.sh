...
backup_signal="$temporary/backup-signal"
make_case "$backup_signal" success
cat >"$backup_signal/interrupt-backup" <<'SH'
#!/bin/bash
printf '%s\n%s\n' "$1" "$2" >"$TEST_BOOTSTRAP_CASE/temporary-paths"
kill -TERM "$PPID"
SH
chmod 755 "$backup_signal/interrupt-backup"
FACTORY_RELEASE_BOOTSTRAP_DURING_BACKUP="$backup_signal/interrupt-backup" \
  run_case "$backup_signal" success \
  && fail "backup signal unexpectedly succeeded"
assert_rolled_back "$backup_signal"
while IFS= read -r temporary_path; do
  [ ! -e "$temporary_path" ] || fail "temporary path survived backup signal: $temporary_path"
done <"$backup_signal/temporary-paths"
