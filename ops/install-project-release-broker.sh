...
SOURCE_ACCEPTANCE=${3:-"$(CDPATH= cd -- "$(dirname -- "$0")" && pwd)/factory-live-acceptance"}
...
ACCEPTANCE_TARGET=${FACTORY_LIVE_ACCEPTANCE_BIN:-/usr/local/lib/factory-live-acceptance}
...
mkdir -p "$(dirname "$BINARY_TARGET")" "$(dirname "$UNIT_TARGET")" "$(dirname "$PILOT_DROPIN_TARGET")" "$(dirname "$ACCEPTANCE_TARGET")"
...
acceptance_tmp=$(mktemp "$(dirname "$ACCEPTANCE_TARGET")/.factory-live-acceptance.XXXXXX")
cleanup() { rm -f -- "$binary_tmp" "$unit_tmp" "$dropin_tmp" "$acceptance_tmp"; }
...
if [ -n "$OWNER" ]; then
  ...
  install -o "${OWNER%:*}" -g "${OWNER#*:}" -m 755 "$SOURCE_ACCEPTANCE" "$acceptance_tmp"
  chown "${OWNER%:*}:${OWNER#*:}" "$dropin_tmp"
else
  install -m 755 "$SOURCE_BINARY" "$binary_tmp"
  install -m 644 "$SOURCE_UNIT" "$unit_tmp"
  install -m 755 "$SOURCE_ACCEPTANCE" "$acceptance_tmp"
fi
...
"$SYNC" -f "$(dirname "$BINARY_TARGET")" "$(dirname "$UNIT_TARGET")" "$(dirname "$PILOT_DROPIN_TARGET")" "$(dirname "$ACCEPTANCE_TARGET")"
