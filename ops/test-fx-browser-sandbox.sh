...
exec /usr/bin/id "$@"
EOF
for command in installer checker launcher; do
...
PATH="$temporary/bin:/usr/bin:/bin" "$temporary/fx" factory browser-sandbox run --proxy-port=1234 -- /bin/true

expected=$(printf '%s\n' 'installer ' 'checker ' 'launcher run --proxy-port=1234 -- /bin/true')
[ "$(cat "$temporary/events")" = "$expected" ]
echo "PASS: fx устанавливает, проверяет и запускает browser-sandbox"
