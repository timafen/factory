...
validate_target_directory_chain() {
  local target=$1 directory path canonical owner mode
  directory=$(dirname -- "$target")
  case "$directory" in
    /*) ;;
    *) directory=$PWD/$directory ;;
  esac
  canonical=$(/usr/bin/readlink -m -- "$directory") || return 1
...
# The privileged bootstrap must validate every destination chain before
# creating a directory or temporary file. Ordinary candidate updates retain
# their existing configurable ownership contract.
if [ "$BOOTSTRAP" = 1 ]; then
  for target in "${targets[@]}"; do
    validate_target_directory_chain "$target" \
      || { echo "unsafe target directory chain: $(dirname -- "$target")" >&2; exit 1; }
  done
  for target in "${targets[@]}"; do
    directory=$(dirname -- "$target")
    ...
    chmod 755 -- "$directory"
    validate_target_directory_chain "$target" \
      || { echo "target directory became unsafe: $directory" >&2; exit 1; }
  done
fi
