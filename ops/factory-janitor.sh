...
# Healthy workers own their retained results. Never feed them into quarantine;
# notify the owner once for each exact retained-worktree snapshot instead.
...
try: data = json.loads(urllib.request.urlopen(sys.argv[1] + "/workers", timeout=20).read())
...
for worker in data.get("workers", []):
    ...
    for retained in worker.get("retained_worktrees") or []:
        fields = [worker.get("name", ""), retained.get("attempt_id", ""), retained.get("repository_id", ""), retained.get("path", ""), retained.get("reason", "")]
        print(*fields, hashlib.sha256(json.dumps(fields, separators=(",", ":")).encode()).hexdigest(), sep="\t")
PY
)
