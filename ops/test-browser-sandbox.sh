...
chain_worker_pid=
chain_browser_pid=
baseline_namespaces=$(ip netns list | awk '$1 ~ /^factory-browser-/ { print $1 }' | sort)
...
cleanup_created_namespaces() {
  local namespace member launcher_pid
  while read -r namespace; do
    [ -n "$namespace" ] || continue
    if printf '%s\n' "$baseline_namespaces" | grep -Fx "$namespace" >/dev/null; then
      continue
    fi
    while read -r member; do
      terminate_pid_group "$member"
    done < <(ip netns pids "$namespace" 2>/dev/null || true)
    launcher_pid=${namespace#factory-browser-}
    terminate_pid_group "$launcher_pid"
    ip netns delete "$namespace" >/dev/null 2>&1 || true
  done < <(ip netns list | awk '$1 ~ /^factory-browser-/ { print $1 }')
}

cleanup() {
  terminate_pid_group "$cancel_launcher_pid"
  terminate_pid_group "$browser_pid"
  terminate_pid_group "$chain_worker_pid"
  terminate_pid_group "$chain_browser_pid"
  cleanup_created_namespaces
...
FACTORY_BROWSER_USER=${FACTORY_BROWSER_USER:-factory} \
  setsid "$launcher" run "--proxy-address=$proxy_address" "--proxy-port=$proxy_port" -- "$temporary/stubborn.sh" "$temporary/stubborn.pid" \
...
# After the new worker is installed, the release helper points this check at
# it. This exercises the real Go -> sudo/fx -> launcher composition with a
# Chromium stand-in that never exits on TERM.
if [ -n "${FACTORY_BROWSER_WORKER:-}" ]; then
  [ -x "$FACTORY_BROWSER_WORKER" ] \
    || { echo "factory-worker for browser chain check is not executable" >&2; exit 7; }
  : >"$temporary/chain-browser.pid"
  chmod 666 "$temporary/chain-browser.pid"
  cat >"$temporary/chain-stubborn.sh" <<SH
#!/bin/sh
printf '%s\n' "\$\$" >"$temporary/chain-browser.pid"
trap '' TERM
while :; do sleep 1; done
SH
  chmod 755 "$temporary/chain-stubborn.sh"
  before_namespaces=$(ip netns list | awk '$1 ~ /^factory-browser-/ { print $1 }' | sort)
  browser_user=${FACTORY_BROWSER_USER:-factory}
  id "$browser_user" >/dev/null 2>&1 \
    || { echo "browser chain user $browser_user does not exist" >&2; exit 7; }
  set +e
  setsid runuser -u "$browser_user" -- env \
    FACTORY_BROWSER_EXECUTABLE="$temporary/chain-stubborn.sh" \
    "$FACTORY_BROWSER_WORKER" browser -timeout=150ms -output "$temporary/unused.png" \
    https://staging-automation.tarser.net/ >"$temporary/chain.out" 2>&1 &
  chain_worker_pid=$!
  wait "$chain_worker_pid"
  chain_status=$?
  chain_worker_pid=
  set -e
  [ "$chain_status" -ne 0 ] || { echo "hung browser chain unexpectedly succeeded" >&2; exit 7; }
  [ -s "$temporary/chain-browser.pid" ] \
    || { echo "hung browser chain did not reach Chromium" >&2; exit 7; }
  chain_browser_pid=$(cat "$temporary/chain-browser.pid")
  if kill -0 "$chain_browser_pid" >/dev/null 2>&1; then
    echo "hung Chromium $chain_browser_pid survived Go cancellation" >&2
    exit 7
  fi
  after_namespaces=$(ip netns list | awk '$1 ~ /^factory-browser-/ { print $1 }' | sort)
  [ "$after_namespaces" = "$before_namespaces" ] \
    || { echo "browser namespace survived full cancellation chain" >&2; exit 7; }
  chain_browser_pid=
  echo "PASS: Go cancellation waited for launcher-owned hung Chromium and namespace cleanup"
fi
