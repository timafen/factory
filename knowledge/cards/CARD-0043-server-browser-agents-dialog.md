...
- Status: Implemented + Tested — блокирующее SSRF-замечание устранено; готово к Review.
- Branch: `factory/4663ef3e-9db-9a8573c2-cbb`.
- Head commit: `6355adb` (`Дать агентам безопасно смотреть утверждённый стенд`).
- Specification: `knowledge/specs/server-browser-for-agents-and-dialog.md`.
- What changed: IPv6 после прежнего deny-list разрешён только в `2000::/3`;
  `fec0::1` и `4000::1` закреплены отрицательными тестами.
- Evidence: `go test ./...` — PASS; UI — 119/119 PASS; TypeScript, ESLint
  и production build — PASS; целевой diff содержит только 22 файла задачи.
