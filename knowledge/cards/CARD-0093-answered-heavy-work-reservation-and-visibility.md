...
Evidence: `python3 -m unittest pilot.test_pilot.AnsweredWorkReservationTests` → 2 passed; `npx vitest run src/Answer.test.tsx src/Overview.test.ts src/WorkView.test.tsx` → 39 passed; `npx tsc -p tsconfig.app.json --noEmit` → passed.
