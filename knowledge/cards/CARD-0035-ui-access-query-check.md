...
- Status: SPECIFIED — первый узкий срез восстановления общих UI-проверок.
- Branch: `factory/e9444636-89f-0c857dea-044`.
- Scope: только ошибка `react-hooks/set-state-in-effect` в `web/src/Access.tsx`.
- Next action: Implement выполняет план ниже, не захватывая остальные восемь
  lint-ошибок и четыре устаревших component-теста.
