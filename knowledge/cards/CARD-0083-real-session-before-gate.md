...
|---|---|---|
| Настоящий gate возвращает статус через kernel wait | `bash ops/test-fx-factory-release.sh` | PASS: failing gate возвращает ошибку и установка не выполняется |
| PATH не подменяет launcher | adversarial `setsid` в целевом suite | PASS: PATH-файл не запускается; возвращается `5` |
| Соседняя функциональность не регрессировала | `just check` | PASS: format, vet, vulnerability scan, staticcheck и Go tests |
