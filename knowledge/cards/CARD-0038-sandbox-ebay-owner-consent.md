...
- Status: In progress — Factory готов, живой OAuth/callback-контур не подтверждён.
- Branch: `factory/f3c8803b-097-bdc90dfa-e90`.
- Head commit: `50a9bfb` — экран, API и точный seller bridge.
- What changed: `/sandbox-keys` запускает и проверяет согласие без передачи
  секретов; `ops/fx` принимает только точный start или одиночный status ID.
- Evidence: shell allowlist, Go, 69/69 UI, TypeScript, build и lint — успешно;
  установленный staging `fx` пока отклоняет `--interactive-bootstrap`.
- One next action: реализовать и штатно выкатить OAuth/callback в
  `tarser-operations`, обновить staging `fx` и пройти полный seller smoke.
