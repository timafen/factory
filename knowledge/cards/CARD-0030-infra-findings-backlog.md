...
### 2026-08-10 — Права конфигурации Пилота
На ветке `factory/24b091e7-9de-eae3e1ed-ab5` Python-запись `pilot/config.json` теперь создаёт временный файл с mode 0600 перед atomic replace.
Новые тесты покрывают новый файл, замену ранее доступного 0644 файла и сохранение обычного umask для несекретного JSON; Go HTTP PUT также проверяет 0600.
Доказательство: `python3 -m unittest pilot.test_pilot` (102 tests), `go test ./...` и `go build ./cmd/factory-server` — PASS.
Открытый риск: права уже существующего конфига исправляются при следующем Python- или Go-сохранении; на сервере текущий файл уже операционно приведён к 0600.
