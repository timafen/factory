...
На ветке `factory/54a5b733-fe2-21bd2cdb-cbf` постановщик связывается с `works.json` по базовому названию, а текущий этап извлекается из заголовка задачи; контракт и тест больше не содержат выдуманного поля `stage`.
Доказательство: целевой Go-тест, `npx vitest run src/Overview.test.tsx` — 5/5; `npx tsc -p tsconfig.app.json --noEmit`, ESLint изменённых файлов и `npm run build` — PASS.
Открытый риск: общий suite (72/87), lint и `go test ./internal/controlplane` падают на существующих ошибках свежего `origin/main` вне файлов этой задачи.
