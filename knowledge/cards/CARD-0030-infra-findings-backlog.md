...
### 2026-08-08 — Общие проверки на чистом main для экрана настроек
На `origin/main` (`4aff94b`) полный Vitest воспроизводит 15 падений неизменённого `App.test.tsx`: 67/82 теста проходят; нужна отдельная синхронизация моков каталога репозиториев.
Там же `go test ./...` не компилирует `internal/controlplane/pilot_config_test.go` после смены формы `PilotConfig.Stages`; это отдельная работа по обновлению тестовой фикстуры.
На ветке `factory/f78ab595-032-b4ace846-1ab` несовпавший 404 `TestHTTPManagedRepositoryCatalog` исправлен восстановлением маршрута, полный Go-набор проходит.
Риск: общий UI-набор и чистый Go-набор main останутся красными до исправления этих независимых регрессий.
