...
### 2026-08-08 — Обзор активной работы
На ветке `factory/536dc9b8-bab-cd5b0b4d-3fd` экран Обзор показывает название активной работы, постановщика и серверный этап; при недоступной метаинформации сама работа остаётся видимой.
Dashboard API вычисляет `running_count` по списку, а UI также безопасно использует длину списка, поэтому заголовок и счётчик не противоречат карточкам ниже.
Доказательство: точечные Go/Vitest-тесты, `go build ./...`, frontend build и ESLint изменённого теста — PASS.
Открытый риск: полные проверки свежего `main` уже падают на несвязанных ожиданиях старого UI, catalog route и существующих lint-нарушениях.
