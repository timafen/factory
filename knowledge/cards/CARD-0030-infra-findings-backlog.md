...
### 2026-08-10 — Эпики переживают очистку истории задач
На ветке `factory/be2db5ef-76a-dcdec47d-671` коммит `beb8dc2` сохраняет receipt завершения подзадачи в JSON эпика и восстанавливает только подтверждённые merge из канонического журнала.
Доказательство: `python3 -m unittest pilot.test_pilot -v` — PASS, 114 тестов; `just build` — PASS.
Открытый риск: старые merge receipt без временной зоны читаются как локальное серверное время; новые записи Пилот пишет в UTC.
