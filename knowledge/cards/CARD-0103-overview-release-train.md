...
### 2026-08-13 — Specification: стабильность теста N+1

Подготовлено продолжение для test-only исправления. Причина нестабильности —
гонка между отображением секции и применением асинхронного mock dashboard, а не
часы машины. Реализация должна ждать фиксированный текст длительности текущего
поезда внутри региона, затем проверять пассажира N+1 и отсутствие выдуманного
времени; `Overview.tsx` менять нельзя. Обязательная проверка — десять
последовательных filtered-прогонов и полный `Overview.test.ts`.

Спецификация: `knowledge/specs/overview-release-train-test-stability.md`.
