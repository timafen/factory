...
- Head commit: `a1a5e24` (состояние поставки после rebase, проверенное перед записью).
...
- Evidence: оба JSON подтвердили 12/120; живой процесс перечитывает абсолютный
  `CONF_PATH` каждый цикл; 32 теста пилота, lint и production build — OK.
...
После rebase на свежий `origin/main` финальная проверка снова подтвердила 32/32
теста пилота, успешные lint и production build. Общий веб-набор: 98 тестов
прошли, 3 существующих теста `Dialog.test.tsx` упали; веб-файлов в поставке нет.
