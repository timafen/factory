...
Утверждённые ограничения: максимум 5 файлов, 10 MiB на файл. Отклонять пустые
файлы, дубликаты IDs, исполняемые расширения и превышение лимитов до создания
задачи. Файлы хранятся под каталогом данных Factory в подпапке с ID задачи;
worker скачивает их по lease-защищённому маршруту и сверяет размер и SHA-256.

## Plan

1. Утвердить лимиты и backing storage; вложения голосового эпика оставить за
   границей первого выпуска, пока не утверждена семантика дочерних задач.
...
4. **Limits.** Владелец утвердил 5 файлов по 10 MiB, файловое хранение под
   `/opt/factory-data` в подпапке ID задачи и запрет исполняемых файлов.
