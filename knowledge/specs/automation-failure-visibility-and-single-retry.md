...
## Проверяемая передача в Implement

Ниже перечислены все файлы реализации. Обязательная команда выражает
пользовательскую суть работы: первый провал schedule Automation ставит тот же
запуск в очередь ровно один раз, а повторные terminal-события не создают ещё
одну попытку. До Implement этот именованный тест отсутствует и потому служит
проверяемым красным контрактом, а после реализации команда обязана завершаться
с кодом 0.
