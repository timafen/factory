...
Готовность реализации не оценивается по наличию ручного runtime на хосте.
Обязательная целевая проверка должна в одной изолированной fixture воспроизвести
чистый штатный релиз, удалить build checkout и подтвердить сигнатуру `%PDF-` у
отчёта, а также не публиковать новое browser-поколение при ошибке подготовки.
Ниже перечислены исчерпывающие файлы реализации и единственная обязательная
команда приёмки.
