...
Ниже перечислены все файлы будущей реализации. Обязательная команда —
целевой новый browser-тест; после реализации она должна завершаться с кодом 0
и доказывать взаимодействие с реальными `/plan` и `/alerts`.
