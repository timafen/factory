...
test("edits pilot settings from the Settings screen", async ({ page }) => {
  ...
  await expect(page.getByRole("heading", { name: "Уведомления и модели" })).toBeVisible();
  const poll = page.getByRole("spinbutton", { name: /^Интервал проверки, секунд/ });
  ...
  await page.reload();
  await expect(page.getByRole("spinbutton", { name: /^Интервал проверки, секунд/ })).toHaveValue("15");
  browser.assertClean();
});
