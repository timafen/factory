...
export function buildWorkGroups(tasks: Task[], verdicts: Record<string, Verdict>, questions: Question[],
                                works: Record<string, WorkMeta> = {}): WorkGroup[] {
  const openQ = new Set(
  ...
  for (const g of map.values()) {
    ...
    for (const it of g.items) {
      if (!it.stage) continue;
      stageRuns.set(it.stage, (stageRuns.get(it.stage) ?? 0) + 1);
      if (seenStages.has(it.stage)) {
        lap += 1;
        seenStages.clear();
      }
    ...
    if (live) {
      ...
      // «Заново» относится к этапу, который действительно запущен повторно.
      // Этапы справа пока остаются историей прошлого круга, а не текущим ходом.
      if (live.stage && (stageRuns.get(live.stage) ?? 0) > 1) g.reached[live.stage] = "again";
...
function sectionOf(g: WorkGroup): "waiting" | "live" | "done" | "archive" {
...
export function WorkView({
  ...
  const groups = buildWorkGroups(tasks ?? [], verdicts, questions, works);
...
function GroupRow({ g, workerMap, expanded, onToggle, onTask, onAnswer, project }: {
  g: WorkGroup; workerMap: Map<string, Worker>; expanded: boolean;
