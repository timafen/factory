  ...
  return (
    <div className="page">
      ...
      <div className="view-toolbar">
        <p>Повторно используемые инструкции Markdown для задач и автоматизаций с неизменяемыми нумерованными редакциями.</p>
      ...
      {items.length === 0 ? (
        <EmptyState
          ...
          description="Создай проверенные инструкции один раз и закрепляй их при постановке задач."
          action={<button className="button button-primary" onClick={() => setCreateOpen(true)}>Создать сценарий</button>}
        />
      ) : (
        <div className="workflow-list">
          <div className="workflow-table-head"><span>Название</span><span>Редакция</span><span>Статус</span><span>Обновлён</span><span /></div>
          {items.map((workflow) => (
    ...
    <div className="page detail-page">
      ...
      <div className="detail-heading">
        <div>
          ...
          <h1>{current.title}</h1>
          <p>Редакция {current.revision_number} · обновлена {new Date(data.workflow.updated_at).toLocaleString()}</p>
        </div>
        <div className="detail-actions">
          <button className="button button-secondary" onClick={() => setEditing(true)}>
            <Pencil size={14} /> Новая редакция
          </button>
          <button
      ...
      {confirmEnabled !== undefined && (
        <div className="confirm-action workflow-confirm" role="alert">
          <span>{confirmEnabled ? "Включить сценарий для новых задач?" : "Выключить сценарий для новых задач? У существующих задач и повторов сохранится их снимок."}</span>
          <button
            className={confirmEnabled ? "button button-primary" : "button button-danger"}
            disabled={enabled.isPending}
            onClick={() => enabled.mutate(confirmEnabled)}
          >
            {enabled.isPending ? "Сохраняем…" : `Подтвердить: ${confirmEnabled ? "включить" : "выключить"}`}
          </button>
      ...
      <div className="detail-grid">
        <section className="panel detail-main">
          <PanelHeading title="Текущие инструкции" aside={`Редакция ${current.revision_number}`} />
        ...
        <section className="panel">
          ...
          <dl className="metadata">
            ...
            <div><dt>Редакции</dt><dd>{data.revisions.length}</dd></div>
            <div><dt>Идентификатор</dt><dd className="mono break-anywhere">{data.workflow.id}</dd></div>
          </dl>
        </section>
      </div>
      <section className="panel">
        <PanelHeading title="История редакций" aside={`${data.revisions.length} неизменяемых редакций`} />
        <div className="revision-list">
          {data.revisions.map((revision) => (
            <details key={revision.id} open={revision.id === current.id}>
              <summary>
                <span><strong>Редакция {revision.revision_number}</strong><small>{revision.title} · {new Date(revision.created_at).toLocaleString()}</small></span>
...
function WorkflowForm({
  ...
  const submit = (event: FormEvent<HTMLFormElement>) => {
    ...
    if (!title) nextErrors.title = "Укажи название сценария.";
    ...
    if (!instructions.trim()) nextErrors.instructions = "Укажи инструкции сценария.";
    else if (new TextEncoder().encode(instructions).length > 48 * 1024) nextErrors.instructions = "Инструкции должны быть не больше 48 КиБ.";
    ...
    <div className="modal-layer">
      ...
      <div className="modal" role="dialog" aria-modal="true" aria-labelledby="workflow-form-heading">
        <div className="modal-header">
          <h2 id="workflow-form-heading">{mode === "create" ? "Создать сценарий" : "Создать редакцию"}</h2>
        ...
        <form onSubmit={submit} noValidate>
          <div className="modal-body">
            ...
            <WorkflowField label="Описание" htmlFor={summaryID} error={errors.summary} hint="Необязательно · до 500 символов">
              <textarea id={summaryID} name="summary" rows={2} defaultValue={current?.summary ?? ""} aria-invalid={Boolean(errors.summary)} />
            </WorkflowField>
            <WorkflowField label="Инструкции Markdown" htmlFor={instructionsID} error={errors.instructions} hint="Обязательно · до 48 КиБ">
          ...
          <div className="modal-footer">
            ...
            <button type="submit" className="button button-primary" disabled={save.isPending}>
              {save.isPending ? <><LoaderCircle size={16} className="spin" /> Сохраняем…</> : <><Plus size={16} /> {mode === "create" ? "Создать сценарий" : "Создать редакцию"}</>}
            </button>
          </div>
        </form>
      </div>
