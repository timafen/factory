  ...
  useEffect(() => {
    ...
    const poll = async () => {
      try {
        ...
        if (cancelled) return;
        if (next.operation_id === consent.operation_id) {
          setError("");
          setConsent(next);
        } else {
        ...
        if (cancelled) return;
        setError("Не удалось проверить согласие. Повторяем проверку автоматически.");
        failures += 1;
        timer = window.setTimeout(poll, Math.min(3_000 * 2 ** failures, 12_000));
      }
    };
    timer = window.setTimeout(poll, 3_000);
