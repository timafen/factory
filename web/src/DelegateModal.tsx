...
export function DelegateModal({
  ...
  const submit = (event: FormEvent<HTMLFormElement>) => {
    ...
    else if (Array.from(title).length > 200) nextErrors.title = "Название должно быть не длиннее 200 символов.";
    ...
    if (!Number.isInteger(timeoutSeconds) || timeoutSeconds < 1 || timeoutSeconds > 28_800) {
      nextErrors.timeout = "Выберите время выполнения от одной минуты до восьми часов.";
    }
  ...
  return (
    <div className="modal-layer">
      <button className="modal-scrim" aria-label="Закрыть окно постановки задачи" onClick={onClose} />
        ...
        <form onSubmit={submit} noValidate>
          <div className="modal-body">
            ...
            <Field
              ...
              hint={mode === "auto"
                ? "Автоматический: пилот проведёт задачу по этапам конвейера и выберет модель для каждого. К названию добавится [auto]."
                : "Ручной: на выбранном исполнителе выполнится только этот этап. Следующие шаги выбираешь ты."}
            >
              <select id="delegate-mode" value={mode} onChange={(event) => setMode(event.target.value as "manual" | "auto")}>
                <option value="manual">Ручной — выполнить только этот этап</option>
            ...
            <Field
              label="Сценарий"
              htmlFor={workflowID}
              hint="Без сценария контекст станет полным промптом задачи."
            >
              <select
                id={workflowID}
              ...
              >
                ...
                {(workflows.data ?? []).map((workflow) => (
                  <option key={workflow.id} value={workflow.current_revision.id}>
                    {workflow.current_revision.title} · версия {workflow.current_revision.revision_number}
                  </option>
                ))}
              </select>
            </Field>
            ...
            <Field
              ...
              hint={selectedWorker
                ? workflowRevisionID
                  ? `Factory объединит его с выбранным сценарием для ${runtimeLabel(selectedWorker.runtime)}.`
                  : `Это станет промптом для ${runtimeLabel(selectedWorker.runtime)}.`
                : workflowRevisionID
                  ? "Factory объединит его с выбранным сценарием."
                  : "Это станет промптом выбранной среды исполнителя."}
            >
            ...
            {selectedWorker && !selectedWorker.online && (
              <div className="warning-banner compact"><AlertCircle size={16} /> Исполнитель не в сети. Задача будет ждать в очереди до его возвращения.</div>
            )}
            {selectedWorker?.health === "unhealthy" && (
              <div className="warning-banner compact"><AlertCircle size={16} /> Исполнитель неисправен и не возьмёт задачу, пока не восстановится.</div>
            )}
            <Field label="Репозиторий" htmlFor="delegate-repository" error={errors.repository}>
              <select id="delegate-repository" value={repositoryID} onChange={(event) => setRepositoryID(event.target.value)} disabled={!workerID || repositoryOptions.isPending}>
                ...
                {repositories.map((repo) => <option key={repo.id} value={repo.id} disabled={!repo.ready && !repo.advertised}>{repo.key ? `${repo.key} · ` : ""}{repo.remote_identity}{repo.ready && !repo.advertised ? " · получается по запросу" : ""}{!repo.ready ? ` · ${repositoryReason(repo.reason)}` : ""}</option>)}
              </select>
            </Field>
          ...
          <div className="modal-footer">
            ...
            <button type="submit" className="button button-primary" disabled={isSubmitting || workers.length === 0}>
              {isSubmitting ? <><LoaderCircle size={16} className="spin" /> Ставим…</> : <><Plus size={16} /> Поставить задачу</>}
            </button>
          </div>
        </form>
      </div>
...
function repositoryReason(reason: string): string {
  const labels: Record<string, string> = {
    "Repository routing is disabled.": "маршрутизация выключена",
    "Repository source is not supported for managed acquisition.": "источник не поддерживает управляемое получение",
    ...
    "Another advertised repository uses this routing identity.": "эту идентичность уже использует другой заявленный репозиторий",
    "Worker cannot acquire managed repositories and does not advertise this one.": "исполнитель не может получить этот репозиторий",
    "Managed repository cache and reservations are full.": "кэш и резервирования репозиториев заполнены",
    "Repository retained-worktree capacity is full.": "лимит сохранённых рабочих копий исчерпан",
  };
  return labels[reason] ?? "недоступен";
}
