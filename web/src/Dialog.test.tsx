...
const settings={settings:{brain_chain:[{cli:"codex",model:"first",provider:"openai",note:"Первая модель"},{cli:"claude",model:"second",provider:"anthropic",note:"Вторая модель"}]},version:"v",warnings:[]};
...
it("selects a configured model and sends the full multi-turn history",async()=>{
  ...
  const select=await screen.findByLabelText("Модель для диалога"); expect(select).toHaveValue("first"); await user.selectOptions(select,"second");
  ...
  expect(await screen.findByText("Ответ два")).toBeVisible(); expect(requests[1]).toEqual({model:"second",messages:[{role:"user",content:"Первый вопрос"},{role:"assistant",content:"Ответ один"},{role:"user",content:"Второй вопрос"}]});
});
