...
const models={models:[{cli:"codex",model:"same",provider:"openai",note:"Первая модель"},{cli:"claude",model:"same",provider:"anthropic",note:"Вторая модель"}]};
...
it("previews, sends, and clears a valid screenshot",async()=>{
  const requests: Array<Record<string,unknown>>=[];
  vi.stubGlobal("fetch",vi.fn(async(input:RequestInfo|URL,init?:RequestInit)=>{
    if(String(input).includes("models")) return new Response(JSON.stringify(models),{status:200,headers:{"Content-Type":"application/json"}});
    requests.push(JSON.parse(String(init?.body)));
    return new Response(JSON.stringify({message:{role:"assistant",content:"Вижу снимок"},model_label:"Первая модель"}),{status:200,headers:{"Content-Type":"application/json"}});
  }));
  render(<Dialog/>); const user=userEvent.setup(); await screen.findByLabelText("Модель для диалога");
  ...
  await user.type(screen.getByLabelText("Ваш вопрос"),"Что здесь?"); await user.click(screen.getByRole("button",{name:"Отправить"}));
  expect(await screen.findByText("Вижу снимок")).toBeVisible();
  expect(requests[0]).toMatchObject({screenshot:{name:"screen.png",content_type:"image/png"}});
  expect(screen.queryByAltText("Скриншот: screen.png")).not.toBeInTheDocument();
  expect(screen.getByLabelText("Скриншот к вопросу")).toHaveValue("");
});

it("clears an earlier screenshot when a new file is invalid",async()=>{
  ...
  await user.upload(input,new File(["png"],"good.png",{type:"image/png"})); expect(await screen.findByAltText("Скриншот: good.png")).toBeVisible();
  fireEvent.change(screen.getByLabelText("Скриншот к вопросу"),{target:{files:[new File(["gif"],"bad.gif",{type:"image/gif"})]}});
  expect(await screen.findByRole("alert")).toHaveTextContent("PNG, JPEG или WebP"); expect(screen.queryByAltText("Скриншот: good.png")).not.toBeInTheDocument();
});
