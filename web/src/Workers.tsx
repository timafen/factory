...
function workerHealthLabel(health: string): string {
  const labels: Record<string, string> = { healthy: "исправен", unhealthy: "неисправен", degraded: "ограничен" };
    ...
    <div className="page">
      ...
      {registered.length === 0 ? (
        <EmptyState
          ...
          description="Запусти исполнителя Factory — он появится здесь после регистрации."
        />
      ) : (
        <>
          ...
          <div className="workers-list">
            <div className="worker-table-head" aria-hidden="true">
              <span>Исполнитель</span><span>Вместимость</span><span>Репозитории</span><span>Версии</span><span>Последний контакт</span><span />
            </div>
            {registered.map((worker) => (
              <button className="worker-row" key={worker.id} onClick={() => onWorker(worker.id)}>
                ...
                <span className="last-seen">{lastSeen(worker.last_heartbeat)}</span>
...
export function WorkerDetail({
  ...
  const latestActiveTask = data.active_count > 1 ? `Последняя · ${activeSessions}` : activeSessions;
    ...
    <div className="page detail-page">
        ...
        <div className="worker-detail-identity">
          ...
          <div>
            <div className="worker-state-line">
              ...
              <span>·</span>
              <span className={data.health === "healthy" ? "healthy-text" : "danger-text"}>{workerHealthLabel(data.health)}</span>
            </div>
            <h1>{data.name}</h1>
            <div className="worker-profile-meta">
              ...
              <span>Последний контакт: {lastSeen(data.last_heartbeat)}</span>
            </div>
          </div>
        </div>
      ...
      {tabPanel("overview",
        <>
          <section className="worker-summary-grid" aria-label="Сводка исполнителя">
            <div><span>Статус</span><strong>{data.online ? "В сети" : "Не в сети"}</strong><small>{workerHealthLabel(data.health)}</small></div>
            <div><span>Сеансы</span><strong>{data.active_count} / {data.capacity}</strong><small>занято</small></div>
            <div><span>Репозитории</span><strong>{data.repositories.length}</strong><small>заявлено</small></div>
          ...
          <div className="worker-overview-layout">
            <section className="panel">
              ...
              <dl className="metadata">
                ...
                <div><dt>Последний контакт</dt><dd>{lastSeen(data.last_heartbeat)}</dd></div>
            ...
            <section className="panel">
              ...
              {data.current_task_title ? (
                ...
                <div className="quiet-empty">Название активной задачи пока не получено.</div>
              ) : (
      ...
      {tabPanel("capabilities",
        <>
          <div className="worker-capabilities-layout">
            <section className="panel">
              ...
              <dl className="metadata">
                ...
                <div><dt>Управляемые репозитории</dt><dd>{data.accepts_managed_repositories ? "поддерживаются" : "не заявлены"}</dd></div>
                <div><dt>Кэш репозиториев</dt><dd>{data.repository_cache_count ?? 0} в кэше</dd></div>
              </dl>
            </section>
            <section className="panel">
              <PanelHeading title="Доступ к источникам" aside={`${data.source_access?.length ?? 0} заявлено`} />
              {(data.source_access ?? []).length === 0 ? (
                <div className="quiet-empty">Исполнитель не заявил доступных источников.</div>
              ) : (
          ...
          <section className="panel">
            <PanelHeading title="Репозитории" aside={`${data.repositories.length} заявлено`} />
            {data.repositories.length === 0 ? (
              <div className="quiet-empty">Исполнитель не заявил локальных копий репозиториев.</div>
            ) : (
      ...
      {tabPanel("settings",
        <>
          <section className="panel execution-settings-card">
            <PanelHeading title="Исполнение" aside="Только чтение" />
            <div className="execution-settings-grid">
              <div className="execution-setting">
                <span>runtime</span>
                <strong>{runtimeLabel(data.runtime)}</strong>
                <small>Один тип runtime для одной идентичности исполнителя.</small>
              </div>
            ...
            <div className="execution-owner-note">
              ...
              <p>
                Factory читает <code>runtime</code> и <code>max_concurrent</code> из TOML исполнителя при запуске.
                Обнови файл и перезапусти исполнителя, чтобы изменить параллельность. runtime неизменяем для
                существующей личности исполнителя; для другого runtime нужна отдельная конфигурация и каталог данных.
              </p>
            </div>
          </section>
          <section className="panel">
            <PanelHeading title="Управление runtime" />
            <p className="settings-copy">
              Модель, усилие рассуждений, скорость и аргументы runtime остаются в установленной конфигурации Codex
              или Claude Code. Factory сообщает возможности исполнения, но не меняет настройки провайдера.
            </p>
          </section>
        </>,
      )}
