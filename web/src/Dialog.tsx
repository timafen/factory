...
export function Dialog() {
  ...
  const screenshotSelection = useRef(0);
  ...
  const send = async () => {
    ...
    if (!content || brainIndex === null || pending || screenshotReading) return;
  ...
  const chooseScreenshot = (file?: File) => {
    if (!file) return;
    const selection = ++screenshotSelection.current;
    if (!(["image/png", "image/jpeg", "image/webp"] as string[]).includes(file.type) || file.size > 4 * 1024 * 1024) {
      setScreenshotReading(false); setScreenshot(null); setScreenshotInputKey((key) => key + 1);
      setError("Прикрепите PNG, JPEG или WebP размером до 4 МБ"); return;
    }
    setError(""); setScreenshot(null); setScreenshotReading(true);
    const reader = new FileReader();
    reader.onload = () => {
      if (selection !== screenshotSelection.current) return;
      setScreenshot({name:file.name, content_type:file.type as DialogScreenshot["content_type"], data:String(reader.result).split(",", 2)[1]});
      setScreenshotReading(false);
    };
    reader.onerror = () => {
      if (selection !== screenshotSelection.current) return;
      setScreenshotReading(false); setError("Не удалось прочитать скриншот. Выберите файл ещё раз");
    };
  ...
  return <section className="dialog-view" aria-labelledby="dialog-title">
    ...
    <div className="field"><span>Скриншот к этому вопросу <small>(необязательно)</small></span>
      <input key={screenshotInputKey} aria-label="Скриншот к вопросу" type="file" accept="image/png,image/jpeg,image/webp" disabled={pending} onChange={(event)=>chooseScreenshot(event.target.files?.[0])} />
      {screenshot && <div className="dialog-screenshot" style={{display:"flex",alignItems:"center",gap:10,marginTop:8}}><img src={`data:${screenshot.content_type};base64,${screenshot.data}`} alt={`Скриншот: ${screenshot.name}`} style={{width:88,height:56,objectFit:"cover",borderRadius:8,border:"1px solid var(--border)"}} /><span>{screenshot.name}</span><button className="button" type="button" disabled={pending} onClick={()=>{setScreenshot(null);setScreenshotInputKey((key)=>key+1);}}>Убрать</button></div>}
    </div>
    <button className="button button-primary" disabled={pending || screenshotReading || brainIndex === null || !question.trim()} onClick={()=>void send()}>{pending ? "Модель думает…" : screenshotReading ? "Читаем скриншот…" : error ? "Повторить" : "Отправить"}</button>
  </section>;
}
