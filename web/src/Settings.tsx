...
const notificationGroups: Array<{key:PilotNotificationGroup;label:string;hint:string;defaultEnabled:boolean}> = [
  ...
  {key:"diag",label:"Диагностика остановок",hint:"Присылать результат разбора застрявшей работы и просьбу о вмешательстве.",defaultEnabled:true},
  {key:"plan",label:"Новые работы в Плане",hint:"Присылать уведомление, когда в Плане появляется новая работа.",defaultEnabled:true},
...
function withNotificationDefaults(settings:PilotSettings):PilotSettings {
  const defaults=Object.fromEntries(notificationGroups.map((group)=>[group.key,group.defaultEnabled]));
  return {...settings,notify_groups:{...defaults,...settings.notify_groups}};
}
...
function SettingsEditor({initial,refresh}:{initial:Awaited<ReturnType<typeof api.pilotSettings>>;refresh:()=>void}) {
  ...
  return <div className="page settings-page">
    <div className="view-header"><div><h1>Настройки</h1><p>Изменения применятся в начале следующего цикла проверки.</p></div><button className="button" onClick={refresh}><RefreshCw size={15}/> Обновить</button></div>
    ...
    {saved && <p className="settings-success">Настройки сохранены.</p>}
    ...
    <SettingsSection title="Работа пилота">
      ...
      <Check label="Учитывать нагрузку хоста" hint="Не запускать новые этапы, пока сервер сообщает о повышенной нагрузке." checked={settings.respect_host_load} onChange={(value) => set("respect_host_load", value)}/>
    ...
    <SettingsSection title="Ограничения и восстановление">
      <OptionalNumberField label="Максимум кругов работы" hint="После скольких кругов одной работы пилот остановит повторения; пусто — значение пилота по умолчанию." value={settings.max_work_rounds} onChange={(value)=>set("max_work_rounds",value)}/>
      <OptionalNumberField label="Спасения после лимита" hint="Сколько раз можно продолжить работу после исчерпания денежного лимита; пусто — значение по умолчанию." value={settings.max_cap_rescues} onChange={(value)=>set("max_cap_rescues",value)}/>
      <OptionalNumberField label="Спасения из цикла" hint="Сколько раз пилот может самостоятельно разорвать цикл; пусто — значение по умолчанию." value={settings.max_loop_rescues} onChange={(value)=>set("max_loop_rescues",value)}/>
      <OptionalNumberField label="Работ одного процесса в день" hint="Дневной предел работ с одним названием; пусто — значение по умолчанию." value={settings.work_day_cap} onChange={(value)=>set("work_day_cap",value)}/>
      <OptionalNumberField label="Всего задач в день" hint="Общий дневной предел создаваемых задач; пусто — значение по умолчанию." value={settings.day_task_cap} onChange={(value)=>set("day_task_cap",value)}/>
      <OptionalNumberField label="Круг до глубокой диагностики" hint="На каком круге пилот должен провести углублённую диагностику; пусто — значение по умолчанию." value={settings.deep_diag_rounds} onChange={(value)=>set("deep_diag_rounds",value)}/>
      <OptionalNumberField label="Дневной лимит использования, %" hint="Максимальная доля дневного ресурса провайдера; пусто — значение по умолчанию." value={settings.day_pct_cap} onChange={(value)=>set("day_pct_cap",value)}/>
    </SettingsSection>
    ...
    <SettingsSection title="Цепочка моделей" wide>
    ...
    <SettingsSection title="Источники данных о продуктах" wide>
    ...
    <SettingsSection title="Заметка о конфигурации"><label className="field settings-full"><span>Заметка о конфигурации</span><textarea aria-label="Заметка о конфигурации" value={settings._note ?? ""} onChange={(event)=>set("_note",event.target.value)} rows={3}/><small className="field-hint">Свободный комментарий для владельца: зачем и когда менялись настройки.</small></label></SettingsSection>
...
function OptionalNumberField({label,hint,value,onChange}:{label:string;hint:string;value?:number;onChange:(value:number|undefined)=>void}) { return <label className="field"><span>{label}</span><input aria-label={label} type="number" step="any" value={value??""} onChange={(event)=>onChange(event.target.value===""?undefined:Number(event.target.value))}/><small className="field-hint">{hint}</small></label>; }
function splitList(value:string) { return value.split(",").map((item)=>item.trim()).filter(Boolean); }
function validate(settings:PilotSettings) { const errors:string[]=[]; const numbers=[settings.poll_seconds,settings.timeout_seconds,settings.max_stage_attempts,settings.max_parallel_subtasks,settings.max_parallel_works,settings.day_cap_usd,...Object.values(settings.stage_base_usd),...Object.values(settings.complexity_factor),...Object.values(settings.work_cap_usd)]; const optionalNumbers=[settings.max_work_rounds,settings.max_cap_rescues,settings.max_loop_rescues,settings.work_day_cap,settings.day_task_cap,settings.deep_diag_rounds,settings.day_pct_cap].filter((value):value is number=>value!==undefined); if([...numbers,...optionalNumbers].some((value)=>!Number.isFinite(value)||value<=0)) errors.push("Все интервалы, лимиты, коэффициенты и бюджеты должны быть положительными."); for(const [label,value] of [["Сервер ntfy",settings.ntfy_server],["Ссылка на чат владельца",settings.owner_chat_url],["Ссылка на интерфейс владельца",settings.owner_ui_url]]) { try { const url=new URL(value); if(!["http:","https:"].includes(url.protocol)) throw new Error(); } catch { errors.push(`${label}: нужен корректный адрес http(s).`); } } if(settings.brain_chain.some((entry)=>!entry.cli.trim()||!entry.model.trim()||!entry.provider.trim())) errors.push("В каждой строке цепочки нужны команда CLI, модель и провайдер."); if(!settings.allow_any_worker && settings.stages.some((row)=>tiers.some((tier)=>!settings.allowed_workers.includes(row.workers[tier])))) errors.push("Каждый назначенный исполнитель должен входить в список разрешённых."); return errors; }
function ErrorMessage({error,conflictAction}:{error:unknown;conflictAction?:()=>void}) { const conflict=error instanceof APIError&&error.status===409; return <div className="settings-errors"><span>{conflict?"Настройки уже изменились. Загрузите свежую версию перед сохранением.":error instanceof Error?error.message:"Не удалось загрузить настройки."}</span>{conflict&&conflictAction&&<button className="button" onClick={conflictAction}>Загрузить свежие настройки</button>}</div>; }
