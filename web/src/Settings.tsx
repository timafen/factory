...
const settingsAnchors = [
  ["pilot", "Работа пилота"], ["routing", "Маршрутизация"], ["automation", "Автоматизация"],
  ["notifications", "Уведомления"], ["models", "Модели"], ["products", "Продукты"],
  ["note", "Заметка"],
] as const;
...
function SettingsEditor({initial,refresh}:{initial:Awaited<ReturnType<typeof api.pilotSettings>>;refresh:()=>void}) {
  ...
  const saveDisabled = errors.length > 0 || save.isPending;
  return <div className="page settings-page">
    ...
    <div className="settings-top-actions">
      <nav className="settings-anchors" aria-label="Разделы настроек">
        {settingsAnchors.map(([id, label]) => <a key={id} href={`#settings-${id}`}>{label}</a>)}
      </nav>
      <button className="button button-primary settings-save-button" disabled={saveDisabled} onClick={saveSettings}><Save size={16}/>{save.isPending ? "Сохраняем…" : "Сохранить настройки"}</button>
    </div>
    ...
    <SettingsSection id="pilot" title="Работа пилота">
    ...
    <SettingsSection id="routing" title="Маршрутизация этапов" wide>
    ...
    <SettingsSection id="automation" title="Автоматизация и бюджеты">
      ...
      <TextField label="Команда автовыпуска Factory" hint="Серверная разрешённая команда, которая выпускает саму Factory после слияния её репозитория." value={settings.deploy_factory_cmd} onChange={(value) => set("deploy_factory_cmd", value)}/>
    </SettingsSection>

    <SettingsSection id="notifications" title="Уведомления и ссылки владельца">
    ...
    <SettingsSection id="models" title="Цепочка моделей" wide>
    ...
    <SettingsSection id="note" title="Заметка о конфигурации"><label className="field settings-full"><span>Заметка о конфигурации</span><textarea aria-label="Заметка о конфигурации" value={settings._note ?? ""} onChange={(event)=>set("_note",event.target.value)} rows={3}/><small className="field-hint">Свободный комментарий для владельца: зачем и когда менялись настройки.</small></label></SettingsSection>
    {errors.length > 0 && <div className="settings-errors">{errors.map((error) => <span key={error}>{error}</span>)}</div>}
    <div className="settings-save"><button className="button button-primary settings-save-button" disabled={saveDisabled} onClick={saveSettings}><Save size={16}/>{save.isPending ? "Сохраняем…" : "Сохранить настройки"}</button></div>
  </div>;
...
function SettingsSection({id,title,children,wide=false}:{id:string;title:string;children:ReactNode;wide?:boolean}) { return <section id={`settings-${id}`} className={`panel settings-section ${wide?"settings-wide":""}`}><h2>{title}</h2><div className="settings-grid">{children}</div></section>; }
...
function validate(settings:PilotSettings) {
  const errors:string[]=[];
  const numbers=[settings.poll_seconds,settings.timeout_seconds,settings.max_stage_attempts,settings.max_parallel_subtasks,settings.max_parallel_works,settings.day_cap_usd,...Object.values(settings.stage_base_usd),...Object.values(settings.complexity_factor),...Object.values(settings.work_cap_usd)];
  if(numbers.some((value)=>!Number.isFinite(value)||value<=0)) errors.push("Все длительности, лимиты, коэффициенты и бюджеты должны быть положительными.");
  for(const [label,value] of [["сервер ntfy",settings.ntfy_server],["чат владельца",settings.owner_chat_url],["интерфейс владельца",settings.owner_ui_url]]) {
    try { const url=new URL(value); if(!["http:","https:"].includes(url.protocol)) throw new Error(); }
    catch { errors.push(`${label} должен быть корректным URL http(s).`); }
  }
  if(settings.brain_chain.some((entry)=>!entry.cli.trim()||!entry.model.trim()||!entry.provider.trim())) errors.push("Для каждой строки цепочки моделей нужны CLI, модель и провайдер.");
  if(!settings.allow_any_worker && settings.stages.some((row)=>tiers.some((tier)=>!settings.allowed_workers.includes(row.workers[tier])))) errors.push("Каждый назначаемый исполнитель должен быть в разрешённом списке, пока исполнители вне списка выключены.");
  return errors;
}
