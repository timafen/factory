...
function validate(settings:PilotSettings) { const errors:string[]=[]; const numbers=[settings.poll_seconds,settings.timeout_seconds,settings.max_stage_attempts,settings.max_parallel_subtasks,settings.max_parallel_works,settings.day_cap_usd,...Object.values(settings.stage_base_usd),...Object.values(settings.complexity_factor),...Object.values(settings.work_cap_usd)]; if(numbers.some((value)=>!Number.isFinite(value)||value<=0)) errors.push("All durations, limits, factors, and budgets must be positive."); for(const [label,value] of [["ntfy server",settings.ntfy_server],["owner chat",settings.owner_chat_url],["owner UI",settings.owner_ui_url]]) { try { const url=new URL(value); if(!["http:","https:"].includes(url.protocol)) throw new Error(); } catch { errors.push(`${label} must be a valid http(s) URL.`); } } if(settings.brain_chain.some((entry)=>!entry.cli.trim()||!entry.model.trim()||!entry.provider.trim())) errors.push("Every brain-chain row needs CLI, model, and provider."); if(!settings.allow_any_worker && settings.stages.some((row)=>tiers.some((tier)=>!settings.allowed_workers.includes(row.workers[tier])))) errors.push("Every routed worker must be in the allowed list while unrestricted workers are disabled."); return errors; }
