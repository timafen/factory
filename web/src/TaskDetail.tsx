...
export function TaskDetail({
  ...
  return (
    <div className="page detail-page">
      <button className="back-button" onClick={onBack}><ArrowLeft size={16} /> Все работы</button>
      <div className="detail-heading">
        <div>
          <StatusBadge state={data.task.state} />
          <h1>{data.task.title}</h1>
          <p>Создано {new Date(data.task.created_at).toLocaleString("ru-RU")}</p>
        </div>
        <div className="detail-actions">
          ...
          {confirmCancel && (
            <div className="confirm-action" role="alert">
              ...
              <button className="button button-secondary" onClick={() => setConfirmCancel(false)}>Продолжить работу</button>
            </div>
          )}
          {retryable && (
            <button className="button button-primary" onClick={() => retry.mutate()} disabled={retry.isPending}>
              ...
              {retry.isPending ? "Повторяем…" : "Повторить задачу"}
            </button>
          )}
          ...
          {confirmDelete && (
            <div className="confirm-action" role="alert">
              <span>Удалить задачу, промпт, попытки и события без возможности восстановления?</span>
              <button
      ...
      {!data.repository_available && (
        <div className="warning-banner"><AlertCircle size={17} /> Репозиторий недоступен выбранному исполнителю. Задача подождёт в очереди.</div>
      )}
      {data.execution.cancellation_requested && data.task.state === "running" && (
        <div className="warning-banner"><Clock3 size={17} /> Отмена запрошена. Исполнитель остановит задачу при следующем сигнале.</div>
      )}
      ...
      <div className="detail-grid">
        ...
        <section className="panel">
          ...
          <dl className="metadata">
            ...
            <div><dt>Тайм-аут</dt><dd>{formatTimeout(data.task.timeout_seconds)}</dd></div>
            <div><dt>Прошло</dt><dd>{taskElapsed(data)}</dd></div>
            <div><dt>Сценарий</dt><dd>{data.workflow ? `${data.workflow.title} · версия ${data.workflow.revision_number}` : "Без сценария"}</dd></div>
          </dl>
        </section>
      </div>
      ...
      <section className="panel progress-panel">
        <PanelHeading title="Ход работы" aside={`${progress.length} обновл.`} />
...
async function loadLaterEvents(attemptID: string, initialAfter: number): Promise<AttemptEvent[]> {
  ...
  for (;;) {
    ...
    if (page.next_after <= requestAfter) {
      throw new Error("Не удалось получить следующие события: пагинация не продвинулась.");
    }
  ...
  return (
    <li>
      ...
      <div>
        ...
        <p>{summary.text}</p>
        <time dateTime={event.server_time}>{new Date(event.server_time).toLocaleTimeString("ru-RU")}</time>
      </div>
    </li>
  );
}
      ...
      {open && (
        <div className="attempt-content">
          <dl className="metadata compact-metadata">
            <div><dt>Начата</dt><dd>{attempt.started_at ? new Date(attempt.started_at).toLocaleString("ru-RU") : "Не начата"}</dd></div>
