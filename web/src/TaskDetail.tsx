    ...
    <div className="page detail-page">
      ...
      <div className="detail-heading">
        <div>
          ...
          <p>Создано {new Date(data.task.created_at).toLocaleString()}</p>
        </div>
        <div className="detail-actions">
          ...
          {confirmCancel && (
            <div className="confirm-action" role="alert">
              ...
              <button className="button button-danger" onClick={() => cancel.mutate()} disabled={cancel.isPending}>
                {cancel.isPending ? "Отменяю…" : "Подтвердить отмену"}
              </button>
          ...
          {retryable && (
            <button className="button button-primary" onClick={() => retry.mutate()} disabled={retry.isPending}>
              ...
              {retry.isPending ? "Повторяю…" : "Повторить задачу"}
            </button>
          )}
          ...
          {confirmDelete && (
            <div className="confirm-action" role="alert">
              <span>Безвозвратно удалить задачу, промпт, попытки и события?</span>
              <button
                className="button button-danger"
                onClick={() => deleteTask.mutate()}
                disabled={deleteTask.isPending}
              >
                {deleteTask.isPending ? "Удаляю…" : "Подтвердить удаление"}
              </button>
      ...
      {!data.repository_available && (
        <div className="warning-banner"><AlertCircle size={17} /> Репозиторий недоступен назначенному исполнителю. Задача в очереди будет ждать.</div>
      )}
      ...
      <section className="panel progress-panel">
        ...
        {!latestAttempt ? (
          ...
          <LoadingLine label="Загружаю ход работы" />
